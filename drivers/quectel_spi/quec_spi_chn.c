...
static unsigned int debug = 0x01;
...
static int spidev_get_slave_ready(struct quectel_spidev *ql_spidev) {
	int pin = gpio_get_value(ql_spidev->gpio_slave_ready);	
	if (debug & 0x01) printk(" %s slave ready:%d flag:%d\n", __func__,pin,ql_spidev->slave_ready_flag);
    //return gpio_get_value(ql_spidev->gpio_slave_ready) && ql_spidev->slave_ready_flag;
    return pin && ql_spidev->slave_ready_flag;
}
...
static void spidev_set_master_ready(struct quectel_spidev *ql_spidev, int ready) {
	...
	if (debug & 0x01) printk(" %s master ready:%d \n", __func__,ready);
}
